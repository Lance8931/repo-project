...
public class StringUtil {
	...
	public static String getSerialOrderNo(int length, String originalNo){
		Integer no = Integer.valueOf(originalNo)+1;
		int oLength = String.valueOf(no).length();
		if(oLength >= length){
			return String.valueOf(no);
		}
		String zero = "0";
		for(int i=1;i<length-oLength;i++){
			zero += "0";
		}
		return zero+no;
	}
