 ...
 * 
 *备注：2016-05-13 修订了_uploadSingle()方法，使上传文件的时候额外提交表单上所有的数据 
 *     2016-05-13 修订了_change()方法，使预览图片前可加载自定义方法 
 *                修订了_initFileActions方法，使删除时可以定义自己的方法
 *                修订了tFooter,更改了预览样式
 *     2016-05-18 修订 tActions 去掉了单个上传按钮
 */
(function (factory) {
    "use strict";
    ...
    tBtnLink = '<a ="javascript:void(0);" tabindex="500" title="{title}" class="{css}" {status}>{icon}{label}</a>';
    ...
    tFooter = '<div class="file-thumbnail-footer">\n' +
        ...
        '    {progress} \n' +
        '</div>' +
        '<div style="margin:5px 10px 0px 10px"><input name="{caption}_hoverTips" style="height:22px;width:55%;margin-bottom: 8px" class="form-control col-sm" placeholder="鼠标悬停提示" aria-describedby="sizing-addon2"/>\n'+
        '<input name="{caption}_imageUrl" style="height:22px;width:90%;" class="form-control col-sm" placeholder="链接地址" aria-describedby="sizing-addon2"/></div>'+
        '   {actions} \n '
    ...
    FileInput.prototype = {
        ...
        _clearPreview: function () {//当所有预览清空时执行
        ...
        _uploadSingle: function (i, files, allFiles) {
            ...
            formdata.append(self.uploadFileAttr, files[i], self.filenames[i]); //input name， 文件， 文件名
        ...
        _initFileActions: function () {//加载图片模板方法
            var self = this;
            if (!self.showPreview) {
                return;
            }
        ...
        _previewFile: function (i, file, theFile, previewId, data) {//加载模板
            if (!this.showPreview) {
                return;
            }
        ...
        _readFiles: function (files) {
            ...
            var self = this, $el = self.$element, $preview = self.$preview, reader = self.reader,
                ...
                throwError = function (msg, file, previewId, index) {
                    ...
                    self._initFileActions();//point 加载图片模板
        ...
        _change: function (e) {
            ...
            var files = isDragDrop ? e.originalEvent.dataTransfer.files : $el.get(0).files;
            ...
            var tfiles, msg, total, isDragDrop = arguments.length > 1, isAjaxUpload = self.isUploadable, i = 0, f, n, len,
                ctr = self.filestack.length,isSingleUpload = isEmpty($el.attr('multiple')), flagSingle = (isSingleUpload && ctr > 0), folders = 0,
            ...
            self._showFolderError(folders);//加载文件
        },
...
/*!
 * upload： _uploadClick --> _uploadSingle-->ajaxSubmit
 * delete：_clearPreview --> reset
*/